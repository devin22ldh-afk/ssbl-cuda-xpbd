...
def _load_library():
    ...
    if not os.path.exists(path):
        raise NativeBackendUnavailable(
            "Missing ABI41 CUDA solver DLL. Install CUDA Toolkit, CMake, and VS Build Tools, then run native/build_abi41.ps1."
        )
...
class NativeXpbdSolver:
    def __init__(self, cloth: ClothBuildData, options: SolverOptions, static_triangles: np.ndarray):
        ...
        self._is_abi41_dll = "abi41" in dll_name
        ...
        if bool(getattr(options, "stretch_optimization_enabled", False)):
            if (_capabilities(self._lib) & _CAP_STRETCH_OPTIMIZATION) == 0:
                raise NativeSolverError(
                    ...
                    "Rebuild native/build_abi41.ps1 to generate the ABI41 DLL."
                )
    ...
    def update_frame_inputs(
        ...
        *,
        ...
        needs_particle_extras = dynamic_particle_count > 0 and not bool(getattr(self, "_is_abi41_dll", False))
