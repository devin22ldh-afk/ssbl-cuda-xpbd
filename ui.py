...
class SSBL_PT_preview_panel(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        if settings.show_advanced_settings:
            ...
            diag_box.label(text=f"constraints_ms: {diagnostics.constraints_ms:.2f}")
            diag_box.label(text=f"volume_ms: {diagnostics.volume_ms:.2f}")
            diag_box.label(text=f"static_collision_ms: {diagnostics.static_collision_ms:.2f}")
            diag_box.label(text=f"dynamic_collision_ms: {diagnostics.dynamic_collision_ms:.2f}")
            diag_box.label(text=f"self_hash_ms: {diagnostics.self_hash_ms:.2f}")
            ...
            diag_box.label(text=f"writeback_frame: {1 if diagnostics.writeback_performed else 0}")
            ...
            diag_box.label(text=f"sleep_regions：{diagnostics.self_sleeping_regions}/{diagnostics.self_active_regions + diagnostics.self_sleeping_regions}")
            diag_box.label(text=f"skipped_self_sources：{diagnostics.self_skipped_sources}")
            diag_box.label(text=f"active_vertices：{diagnostics.self_active_vertices}")
            diag_box.label(text=f"active_samples：{diagnostics.self_active_samples}")
