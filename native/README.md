...
The add-on uses the reconstructed ABI40 solver at
`native/bin/ssbl_xpbd_cuda_abi40.dll` through `ctypes`.
...
For the reconstructed ABI40 smoke build, run:

```powershell
.\build_recon.ps1
```

This builds only the ABI40 target and runs `ssbl_abi41_smoke.exe`, which should
print `SSBL_ABI41_NATIVE_OK` and `SSBL_ABI41_STATIC_SDF_OK`.

The expected ABI40 output DLL is:

```text
native/bin/ssbl_xpbd_cuda_abi40.dll
```

Set `SSBL_NATIVE_DLL_PATH` to load a specific ABI40-compatible candidate DLL.
