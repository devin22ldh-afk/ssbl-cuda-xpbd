...
Pin targets carry per-pin weights derived from vertex-group weights multiplied by the global pin hardness. Python keeps any positive pin weight in the slot data so the native side can apply the same target stream during preview, bake, interactive pinning, and manual native probes.
...
`SsblXpbdFrameInputs` is the per-frame incremental input package. It uses update flags to selectively upload pin targets, pin weights, runtime colliders, static triangles, dynamic triangles, dynamic particles, and force fields.
...
The ABI41 solver applies weighted pin projection through `abi41_pin_project_kernel()`. Effective pin weights below `0.05` are intentionally ignored on the native side, while weights at or above `0.75` align with the Python hard-pin classification used for inverse mass and hidden tether setup.

Contact-heavy substeps can move cloth after the normal constraint passes. When analytic colliders, static SDF, dynamic cloth collision, self collision, signed dynamic collision, or final ABI41 stretch constraints run, the native solver performs a final pin projection polish so pins remain anchored after contact resolution.

The ABI41 contact path uses separate tuning for self-contact previous-position sync, signed dynamic projection relaxation, and outward velocity retention. Treat those constants as stability knobs for contact jitter and penetration recovery; validate them with both scene-level smoke tests and the native contact group probe before changing them.
...
Successful output should include `SSBL_OBJECT_COLLISION_SMOKE`. Set `SSBL_OBJECT_COLLISION_CASES` to a comma-separated subset, for example `ground,wall,static_mesh,moving_static_mesh`, when you need a shorter focused collision pass.
...
Successful output should include `SSBL_NATIVE_CONTACT_GROUP_PROBE`. The probe cases isolate native analytic contacts, static SDF contact, and self vertex-vertex contact through `NativeXpbdSolver`, which is useful when tuning contact grouping, friction, or pin-contact interaction without creating a full Blender test scene.
...
Native rebuild smoke should print `SSBL_ABI41_NATIVE_OK`, `SSBL_ABI41_STATIC_SDF_OK`, and `SSBL_ABI41_PIN_WEIGHT_OK`. Run this path when changing native code, ABI structs, DLL capabilities, or native pin-weight behavior.
...
pin targets 会携带每个 pin 的权重，权重来源是顶点组权重乘以全局 pin 硬度。Python 会把所有正权重 pin 保存在 slot 数据中，因此 preview、bake、interactive pin 和手动 native probe 都会使用同一套 target stream。
...
`SsblXpbdFrameInputs` 是逐帧增量输入数据包。它通过 update flags 选择性上传 pin targets、pin weights、runtime colliders、static triangles、dynamic triangles、dynamic particles 和 force fields。
...
ABI41 解算器通过 `abi41_pin_project_kernel()` 执行加权 pin projection。native 侧会有意忽略低于 `0.05` 的有效 pin 权重；`0.75` 或更高权重则与 Python 侧用于 inverse mass 和 hidden tether 设置的 hard-pin 分类一致。

接触较多的 substep 可能会在普通约束之后再次移动布料。当解析碰撞体、静态 SDF、动态布料碰撞、自碰撞、signed dynamic collision 或最终 ABI41 stretch 约束运行时，native 解算器会执行一次 final pin projection polish，确保 contact resolution 之后 pin 仍保持锚定。

ABI41 contact 路径分别用 self-contact previous-position sync、signed dynamic projection relaxation 和 outward velocity retention 调整稳定性。修改这些常量时，应把它们视为接触抖动和穿透恢复的稳定性旋钮，并同时用场景级 smoke 和 native contact group probe 验证。
...
成功输出应包含 `SSBL_OBJECT_COLLISION_SMOKE`。需要更短的碰撞专项检查时，可将 `SSBL_OBJECT_COLLISION_CASES` 设为逗号分隔的子集，例如 `ground,wall,static_mesh,moving_static_mesh`。
...
成功输出应包含 `SSBL_NATIVE_CONTACT_GROUP_PROBE`。这些 probe 通过 `NativeXpbdSolver` 隔离 native 解析接触、静态 SDF 接触和自 vertex-vertex 接触，适合在不创建完整 Blender 测试场景时调试 contact grouping、摩擦或 pin-contact 交互。
...
native rebuild smoke 应输出 `SSBL_ABI41_NATIVE_OK`、`SSBL_ABI41_STATIC_SDF_OK` 和 `SSBL_ABI41_PIN_WEIGHT_OK`。修改 native code、ABI structs、DLL capabilities 或 native pin-weight 行为时运行这条路径。
